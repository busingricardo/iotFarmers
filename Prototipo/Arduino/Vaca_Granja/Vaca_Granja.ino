...
void subscribe(char *newTopic) {
  String topicStr = createTopicFarm(newTopic);
...
void callback(char *topic, byte *payload, unsigned int length) {
  ...
  const String updateTopicStr = createTopicFarm(CMD_TOPIC);

  String msg = unwrapMessage(payload, length);
  Serial.println(" => " + String(topic) + ": " + msg);
  Serial.println("Topic suscrito"+ String(updateTopicStr));
...
String createTopic(char *topic) {
  String topicStr = String("Vacas") + "/" + topic;
  return topicStr;
}
