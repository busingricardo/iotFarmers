...
#define TEMP_SALA1 "Sala/1/Temp"
#define HUM_SALA1 "Sala/1/Hum"
#define PRES_SALA1 "Sala/1/Pres"
#define CO2_SALA1 "Sala/1/CO2"
#define TVOC_SALA1 "Sala/1/TVOC"
#define LUZ_SALA1 "Sala/1/Lux"

#define TEMP_SALA2 "Sala/2/Temp"
#define HUM_SALA2 "Sala/2/Hum"
#define PRES_SALA2 "Sala/2/Pres"
#define CO2_SALA2 "Sala/2/CO2"
#define TVOC_SALA2 "Sala/2/TVOC"
#define LUZ_SALA2 "Sala/2/Lux"

#define TEMP_EXT1 "Exterior/1/Temp"
#define HUM_EXT1 "Exterior/1/Hum"

#define COMMAND_TOPIC "cmd"
#define TEST_TOPIC "test"

//DEFINE PINS
#define DHT_PIN 17     // Defines pin number to which the sensor is connected
...
void sendData(TimerHandle_t xTimer){
  ...
  publishData(TEMP_SALA1, bmp280Temp);
  publishData(HUM_SALA1, SI702xHum);
  publishData(PRES_SALA1, bmp280Pres);
  publishData(CO2_SALA1, ccs811eCO2);
  publishData(TVOC_SALA1, ccs811eTVOC);
  publishData(LUZ_SALA1, light);

  publishData(TEMP_SALA2, (bmp280Temp * 0.96));
  publishData(HUM_SALA2, (SI702xHum * 1.08));
  publishData(PRES_SALA2, (bmp280Pres * 1.03));
  publishData(CO2_SALA2, (ccs811eCO2 * 0.94));
  publishData(TVOC_SALA2, (ccs811eTVOC * 0.89));
  publishData(LUZ_SALA2, light);

  publishData(TEMP_EXT1, SI702xTemp);
  publishData(HUM_EXT1, humidityDHT);
...
String createTopic(char *topic) {
  String topicStr = String("Factory") + "/" + topic;
  return topicStr;
}
...
void publishFactoryReception(char* Topic) {
  ...
  R1["t_tk1"] = temperatureDHT;
  R1["l_tk1"] = distance * random(1, 2);
  R1["t_tk2"] = SI702xTemp;
  R1["l_tk2"] = distance * random(0.8, 1.8);
  R1["d_cmn"] = distance;
